...
1. **Branch** — Create `enhancement/<slug>` from current HEAD. The branch MUST remain local — do NOT push or create a pull request. The user decides when and how to integrate.
...
**THINK** — Before anything, read: `.lab/results.tsv`, `.lab/log.md` (last 5 entries if 20+), `.lab/branches.md`, `.lab/parking-lot.md`, and in-scope source files. Re-read the execution discipline rules above. Then analyze, hypothesize, formalize your understanding. Check convergence signals. If accumulated code shows unnecessary complexity, invoke both `/simplify` and `/simplifier` — only when justified, not by default. Stay as long as productive.
...
5. NEVER push the branch or create a pull request. The branch stays local. The user decides integration.
</critical>
