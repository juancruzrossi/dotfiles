...
**Step 4 — Plan images.** Plan visual treatments for the deck. At minimum, target the **title slide** (16:9 background that sets the visual tone) and **one full-bleed slide** (immersive background for a key moment). Content slides with conceptual topics also benefit from a 1:1 illustration in the aside area. Use CSS gradients and SVG decorations for backgrounds. See the Proactive Imagery section below for patterns.
...
CSS gradient or background image dominates the viewport. Text overlay with gradient scrim ensuring contrast. Zero slide padding.
...
**Background imagery:** Use CSS gradients and SVG decorations for slide backgrounds. Target these slides in priority order:

1. **Title slide** (always): background treatment that sets the deck's visual tone. Match the topic and palette. Use bold `linear-gradient` or `radial-gradient` with preset colors.
2. **Full-bleed slide** (always if deck has one): immersive background for the deck's visual anchor moment. Style should match the preset — deep gradients for Midnight Editorial, clean geometric SVG for Swiss Clean, terminal-style for Terminal Mono.
3. **Content slides with conceptual topics** (1–2 if the deck has room): illustration in the `.slide__aside` area for slides about abstract concepts using inline SVG.

**CSS gradient backgrounds:** Use the `.slide__bg--gradient` pattern with bold `linear-gradient` or `radial-gradient` backgrounds. The deck should stand on its own visually with CSS and SVG alone.
...
**When to skip images:** Pure structural or data-heavy decks (code reviews, table comparisons) may not need background imagery. Use CSS gradients and SVG decorations instead.
